...
import { createShopifyFulfillment } from "../services/fulfillment.server";
  ...
  try {
    ...
    await prisma.shipmentEvent.create({
      data: {
        ...
        status: "CREADO",
        description: `Envío manual creado. Tracking: ${result.trackingNumber || "pendiente"}`,
        eventDate: new Date(),
      },
    });

    // Sync fulfillment to Shopify (if linked to a Shopify order)
    if (shipment.shopifyOrderId && result.trackingNumber) {
      try {
        ...
        if (fulfillmentResult.success && fulfillmentResult.fulfillmentId) {
          await prisma.shipment.update({
            where: { id: shipment.id },
            data: { shopifyFulfillmentId: fulfillmentResult.fulfillmentId },
          });
        }
      } catch (e) {
        console.error("[Fulfillment] Non-blocking error:", e);
      }
    }
