...
/**
 * Fetches unfulfilled orders from Shopify that don't have DHL shipments yet
 */
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // Check which orders already have DHL shipments
...
/**
 * Creates DHL shipments for selected orders
 */
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!credentials?.verified) {
    return json({ error: "Credenciales DHL no verificadas. Ve a Configuración primero.", results: null });
  }
  ...
  for (const orderId of orderIds) {
    try {
      ...
      // Create shipment in DB (DHL API call happens here when real API is connected)
      // For now, we create in PENDIENTE status — the DHL API service will process it
