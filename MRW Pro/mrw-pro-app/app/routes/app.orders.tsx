...
/**
 * Fetches unfulfilled orders from Shopify that don't have MRW shipments yet
 */
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const enrichedOrders = orders.map((order: any) => {
    ...
    return {
      ...
      trackingNumber: existing?.mrwTrackingNumber || null,
...
/**
 * Creates MRW shipments for selected orders
 */
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!credentials?.verified) {
    return json({ error: "Credenciales MRW no verificadas. Ve a Configuración primero.", results: null });
  }
  ...
  for (const orderId of orderIds) {
    try {
      ...
      if (existing) {
        results.errors.push(`Pedido ${order.name}: ya tiene envío (${existing.mrwTrackingNumber || existing.status})`);
        continue;
      }
      ...
      // Create shipment in DB (MRW API call happens here when real API is connected)
      // For now, we create in PENDIENTE status — the MRW API service will process it
      ...
      results.shipments.push({
        ...
        trackingNumber: shipment.mrwTrackingNumber || "pendiente",
      });
